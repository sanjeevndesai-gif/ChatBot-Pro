...
public class FlowResolver {
    ...
    // ✅ Lazy self proxy for @Cacheable
    private final FlowResolver self;
    ...
    public Map<String, Object> handle(Map<String, Object> convo, String input) {
        ...
        boolean timedOut =
                lastMessageAt != null
                        && lastMessageAt.isBefore(
                                Instant.now().minusSeconds(timeoutSeconds));

        // ✅ Cacheable call through Spring proxy
        Map<String, Object> flow =
                self.getFlowFromCache(String.valueOf(convo.get("flowId")));
        ...
        // Restart logic
        if ((ended || timedOut)
                && !"START".equals(convo.get("currentStep"))) {

            String startStep = (String) flow.get("start");

            @SuppressWarnings("unchecked")
            Map<String, Object> steps =
                    (Map<String, Object>) flow.get("steps");

            @SuppressWarnings("unchecked")
            Map<String, Object> start =
                    (Map<String, Object>) steps.get(startStep);
            ...
            newConvo.put(
                    "message",
                    new HashMap<>(
                            (Map<String, Object>) start.get("message")));
        ...
        Map<String, Object> step =
                (Map<String, Object>) steps.get(stepId);
        ...
        Map<String, Object> ctx =
                (Map<String, Object>) convo.get("context");
        ...
        try {
            ...
            convo.put(
                    "message",
                    new HashMap<>(
                            (Map<String, Object>) step.get("message")));
        ...
        Map<String, Object> nextStep =
                (Map<String, Object>) steps.get(next);
    ...
    public Map<String, Object> getFlowFromCache(String flowId) {
        ...
        Object result =
                mongo.findById(flowId, Map.class, "flows");
    ...
    private String normalizeInput(Object rawInput) {
        ...
        if (rawInput instanceof Map<?, ?> map) {
            ...
            try {
                ...
                var entry =
                        (List<Map<String, Object>>) map.get("entry");

                var changes =
                        (List<Map<String, Object>>) entry.get(0)
                                .get("changes");

                var value =
                        (Map<String, Object>) changes.get(0)
                                .get("value");
                ...
                var text =
                        (Map<String, Object>) messages.get(0)
                                .get("text");
